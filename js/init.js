...
(function($,document,window){
	...
	/* ROTATING COUPONS */
	$(document).ready(function($){
		var w,mHeight,tests = $("#coupon-area");
		w = tests.outerWidth();
		mHeight = 175;
		tests.find(".special").each(function(index){
			$("#s_indexes").find(".s-index:eq(0)").addClass("active"); // Make the first pager active initially
			if(index == 0){
				$(this).addClass("active"); // Make the first slide active initially
			}
			...
			var l = index * w; // Find the left position of each slide
			$(this).css("left",l);
			tests.find("#our_specials").height(mHeight);// Make the height of the slider equal to max height of the slides
		});
		
		$(".s-index").on("click",function(e){ // Clicking action for pagination
			e.preventDefault();
			next = $(this).index(".s-index");
			clearInterval(t_int);// Clicking stops the autoplay
			moveIt(next);
		});
		
		function moveIt(next){ // The main sliding function
			var c = parseInt($(".special.active").removeClass("active").css("left")); // Current position
			var n = parseInt($(".special").eq(next).addClass("active").css("left")); // New Position
			$(".special").each(function(){ // shift each slide
				if(n > c){
					$(this).animate({"left" : "-=" + (n - c) + "px"});
				}else{
					$(this).animate({"left" : "+=" + Math.abs(n - c) + "px"});
			...
			$(".s-index.active").removeClass("active");
			$("#s_indexes").find(".s-index").eq(next).addClass("active");
		}
		
		var t_int = setInterval(function(){ // For autoplay
			var i = $(".special.active").index(".special");
